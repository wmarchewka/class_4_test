...
class SecurityLevel(object):
    ...
    # *******************************************************************************
    def update_gui(self, window=None):
        #SPILOG,TECH,MISC,EEPROM,MAIN,GRAPH,CODERATE,SPEED,CAL,SETUP
        if window is None:
            window = self.window
        else:
            self.window = window
        self.tabs = self.window.tabWidget
        self.log.debug("Setting security level to {}".format(self.level))
        if self.level == "admin":
            self.tabWidget_add("SPILOG")
            self.tabWidget_add("TECH")
            self.tabWidget_add("MISC")
            self.tabWidget_add("EEPROM")
            self.tabWidget_add("MAIN")
            self.tabWidget_add("GRAPH")
            self.tabWidget_add("CODERATE")
            self.tabWidget_add("SPEED")
            self.tabWidget_add("CAL")
            self.tabWidget_add("SETUP")
            self.set_current_index("CODERATE")
        elif self.level == "factory":
            self.tabWidget_remove("SPILOG")
            self.tabWidget_add("TECH")
            self.tabWidget_remove("MISC")
            self.tabWidget_add("EEPROM")
            self.tabWidget_remove("MAIN")
            self.tabWidget_remove("GRAPH")
            self.tabWidget_add("CODERATE")
            self.tabWidget_add("SPEED")
            self.tabWidget_add("CAL")
            self.tabWidget_add("SETUP")
            self.set_current_index("SETUP")
        elif self.level == "technician":
            self.tabWidget_remove("SPILOG")
            self.tabWidget_add("TECH")
            self.tabWidget_remove("MISC")
            self.tabWidget_remove("EEPROM")
            self.tabWidget_remove("MAIN")
            self.tabWidget_remove("GRAPH")
            self.tabWidget_add("CODERATE")
            self.tabWidget_add("SPEED")
            self.tabWidget_remove("CAL")
            self.tabWidget_remove("SETUP")
            window.TECH_FRAME_cs_manual.move(10, 10)
            window.TECH_FRAME_gpio_manual.move(10, 85)
            window.TECH_FRAME_brightness.move(10, 170)
            self.set_current_index("CODERATE")
        elif self.level == "customer":
            self.tabWidget_remove("SPILOG")
            self.tabWidget_remove("TECH")
            self.tabWidget_remove("MISC")
            self.tabWidget_remove("EEPROM")
            self.tabWidget_remove("MAIN")
            self.tabWidget_remove("GRAPH")
    ...
    # *******************************************************************************
    def set_current_index(self, tabname):
        index = self.get_index_from_name(tabname)
        self.window.tabWidget.setCurrentIndex(index)

    # *******************************************************************************
    def get_index_from_name(self, tabname):
        page = self.window.tabWidget.findChild(QtWidgets.QWidget, tabname)
        index = self.window.tabWidget.indexOf(page)
        return index
    # *******************************************************************************
    def tabWidget_remove(self, tabname):
        page = self.window.tabWidget.findChild(QtWidgets.QWidget, tabname)
    ...
    def tabWidget_change(self, tabname, window):
     ...
     # *******************************************************************************
    def tabWidget_add(self, tabname):
        tmp_page = self.window.tabWidget.findChild(QtWidgets.QWidget, tabname)
        self.window.tabWidget.addTab(tmp_page, tabname)

    # *******************************************************************************
    def index_tab_pages(self, window):
        self.tab_pages = []
        count = window.tabWidget.count()
        self.log.debug("Tabwidget Count:{}".format(count))
